        ...
        <div class=' divide'>
            <div style='margin-top: 2vw;'>
                ...
                <div class='fs-16px'>
                    ...
                    <p style='text-align: center;'><span style='display: inline-block;' >$$ y = (a + bx_{0}) \\pm \\sqrt{\\frac{1}{n} + \\frac{(x_{0} - \\overline{x})}{S_{xx}}} \\sqrt{\\frac{SS_{r}}{n-2}} t_{\\alpha_{h}, n-2} $$</span></p>
                </div>
...
function calculate_i(num) {
    ...
    left -= Math.sqrt((1 / n) + ((num - X_avg) / sxx)) * Math.sqrt((ssr) / (n - 2)) * t2_n_2;
    let right = (mat_x[1] + (mat_x[0] * num));
    right += Math.sqrt((1 / n) + ((num - X_avg) / sxx)) * Math.sqrt((ssr) / (n - 2)) * t2_n_2;
    return [left, right];
}
